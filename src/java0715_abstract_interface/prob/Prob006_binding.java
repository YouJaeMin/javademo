...
public class Prob006_binding {
	public static void main(String[] args) {
		...
		for (int i = 0; i < em.length; i++) {
			int  bonus = 0;
			if (em[i] instanceof Engineer) {
				Engineer en = (Engineer) em[i];

				bonus = (int) (en.salary * 0.13);

				System.out.println(en.toString() + "\t" + bonus);

			} else if (em[i] instanceof Developer) {
				Developer de = (Developer) em[i];

				bonus = (int) (de.salary * 0.15);

				System.out.println(de.toString() + "\t" + bonus);
			}
		}
